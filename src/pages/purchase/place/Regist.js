					...
					<tbody>
						{form.placeDetails.map((detail, index) => (
							<tr key={index}> 
								...
								<td><input type="text" value={detail.placeNote} onChange={(e) => handleChange(e, index, 'placesNote')}/></td>
								<td onClick={() => removeDetail(index)}> - </td>
							</tr>
						))}
					</tbody>
				</Table>
				<MainButton	className='mainButton' onClick={ onClickPlaceRegistHandler }>	등록 </MainButton>
